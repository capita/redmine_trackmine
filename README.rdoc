...
The plugin integrates Redmine projects (or subprojects)
with Pivotal Tracker projects (or labels).

== Important Notice!

We are still working on the plugin. Please be aware that the Trackmine will go through a number of changes over the next few weeks.
...
Install gems on a server 

  $ bundle install

or you can use gems bundled with a plugin if you don't have permission to install them on a server

  $ bundle install --path vendor/plugins/redmine_trackmine/vendor/bundle
...
* Make sure you have 'Pivotal Story ID' (field_format: int, Trackers: Bug, Support, Feature) custom field for Redmine Issue.
